...
// Same table as Tom-evnut/BMWPhevBMS / our Due V2 (poly 0x1D, init 0xFF)
static const uint8_t crc_1d[256] = {
...
const uint8_t BmwCrc::finalxor[12] = {0xCF, 0xF5, 0xBB, 0x81, 0x27, 0x1D,
                                      0x53, 0x69, 0x02, 0x38, 0x76, 0x4C};

uint8_t BmwCrc::calc(uint16_t id, const uint8_t *buf, uint8_t len, int nextmes) {
  uint8_t crc = 0xFF;
  crc = crc_1d[crc ^ (uint8_t)(id >> 8)];
  crc = crc_1d[crc ^ (uint8_t)(id & 0xFF)];
  for (uint8_t i = 0; i < (uint8_t)(len - 1); i++)
    crc = crc_1d[crc ^ buf[i]];
  return (uint8_t)(crc ^ finalxor[nextmes % 12]);
}
